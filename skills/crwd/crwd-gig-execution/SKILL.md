...
1. **Confirm the gig and its type** (live `irl` vs online) with `crwd_db` `get_gig_details`.
   ...
   When listing or naming a gig, copy the `name` / `gig_name` field **verbatim** from
   `crwd_db` — it is already a markdown link to the gig page. Do not paraphrase,
   bold, or rebuild gig titles. Never hyperlink store names or product names.
