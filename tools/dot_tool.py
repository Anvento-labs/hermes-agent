...
DOT_SCHEMA = {
    ...
    "description": (
        ...
        "CRWD sends members money for TWO different reasons — product funds (to buy the "
        "product with) and the gig payout (the fee for completing it) — and Dot has no "
        "field saying which one a transfer is. Two traps: (1) a transfer's `type` field "
        "reading \"payout\" only means money going out to the member, and BOTH kinds "
        "arrive that way, so it never tells you which this is; (2) Dot amounts are in "
        "CENTS while CRWD's payout/product-funds figures are in dollars, so convert "
        "before comparing. To classify, read the free text CRWD attached (`metadata`, or "
        "the payout link's `memo` when `payout_link_id` is set) and compare the amount "
        "against that gig's effective_payout vs effective_product_funds from crwd_db. If "
        "neither settles it, report the amount, date and status WITHOUT naming which "
        "payment it was, and hand off — never guess a payment's purpose. Earnings totals "
        "count payouts only; product funds are spending money, not income. "
